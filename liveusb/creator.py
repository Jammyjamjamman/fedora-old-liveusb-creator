...
class LiveUSBCreator(object):
    ...
    def setup_olpc(self):
        ...
        olpc_cfg = open(os.path.join(self.dest, 'boot', 'olpc.fth'), 'w')
    ...
    def get_kernel_args(self):
        ...
        args = []
        cfg = open(os.path.join(self.dest, 'isolinux', 'syslinux.cfg'))
    ...
    def get_release_from_iso(self):
        """ If the ISO is for a known release, return it. -modified"""
        isoname = os.path.basename(self.iso)
        for release in releases:
            rel = release.get('url')
            if rel is not None and os.path.basename(rel) == isoname:
                return release
    ...
    def _set_drive(self, drive):
        ...
        if drive not in self.drives:
            found = False
            for key in list(self.drives.keys()):
...
class LinuxLiveUSBCreator(LiveUSBCreator):
    ...
    def detect_removable_drives(self, callback=None):
        ...
        def handleAdded(name, device):
            ...
            if (not drive['Removable'] or
                drive['Optical'] or
                    (drive['ConnectionBus'] != 'usb' and
                     drive['ConnectionBus'] != 'sdio')):
            ...
            data = {
                ...
                'mount': list(map(strify, fs['MountPoints'])),
                'size': int(blk['Size']),
            }
            ...
            parent_obj = self.bus.get_object("org.freedesktop.UDisks2", partition['Table'])
    ...
    def get_mbr(self):
        ...
        mbr = ''.join(['%02X' % x for x in drive.read(2)])
        drive.close()
    ...
    def mbr_matches_syslinux_bin(self):
        ...
        syslinux mbr.bin
        """
        mbr_bin_path = self._get_mbr_bin()
        if mbr_bin_path is None:
            return False
        mbr_bin = open(self._get_mbr_bin(), 'rb')
        mbr = ''.join(['%02X' % x for x in mbr_bin.read(2)])
    ...
    def get_extlinux_version(self):
        ...
        if p.returncode == 0:
            version = int(str(err).split()[1].split('.')[0])
