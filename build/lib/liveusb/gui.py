...
class Release(QObject):
    ...
    def size(self):
        if 'x86_64' in list(self._data['variants'].keys()):
            return self._data['variants']['x86_64']['size']
        return self._data['variants']['']['size']

    @size.setter
    def size(self, value):
        if self._data.get('x86_64') is None:
            self._data['x86_64'] = {}
        val = self._data['x86_64'].get('size')
        if value != val:
    ...
    @pyqtProperty(str, constant=True)
    def arch(self):
        return list(self._data['variants'].keys())
...
class ReleaseListProxy(QSortFilterProxyModel):
    ...
    def possibleArchs(self):
        return list(self._archMap.keys())

    @pyqtProperty(str, notify=archChanged)
    def archFilter(self):
        for name, abbrs in list(self._archMap.items()):
